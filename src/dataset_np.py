...
class NPImageDataset(Dataset):
    ...
    def __init__(
        ...
        # Special flag: when using LFW test set, filenames won't match mask stems.
        # In that case we pair images and masks purely by sorted order.
        self.lfw_test = lfw_test
        ...
        if self.lfw_test:
            # For LFW-style test data, use ordered pairing instead of stem matching.
            self.mask_pairs = self._build_pairs_ordered(self.images, self.masks)
            # Restrict images to those that have a corresponding mask
            self.images = sorted(self.mask_pairs.keys())
        else:
        ...
        # For standard datasets we require an exact match between images and masks.
        # For LFW test mode we allow partial pairing (only images with masks are used).
        missing = [img for img in self.images if img not in self.mask_pairs]
        if missing and not self.lfw_test:
    ...
    @staticmethod
    def _build_pairs_ordered(images, masks) -> Dict[str, str]:
        """
        Map images to masks by sorted order rather than by filename stem.

        This is useful for LFW-style test sets where image filenames (e.g.
        'Abdullah Gul_0.jpg') do not share stems with mask files
        (e.g. 'mask_00000.npz'), but both lists are aligned by index.
        """
        paired = {}
        n = min(len(images), len(masks))
        for i in range(n):
            paired[images[i]] = masks[i]
        return paired
