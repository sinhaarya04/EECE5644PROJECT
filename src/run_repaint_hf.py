...
def load_mask(mask_path):
    """Load mask from local path and invert for Hugging Face RePaintPipeline
    
    Our mask format: White (255) = masked, Black (0) = known
    HF RePaint expects: Black (0) = masked, White (255) = known
    So we need to invert: 255 - mask
    """
    mask = Image.open(mask_path).convert("L").resize((256, 256))
    # Convert mask to binary (0 or 255)
    mask_array = np.array(mask)
    mask_array = (mask_array > 127).astype(np.uint8) * 255
    # INVERT: Our format (255=masked) -> HF format (0=masked)
...
def run_repaint_hf(
    ...
    mask_image = load_mask(mask_path)  # Inverted for pipeline
    
    # Load original mask for visualization (before inversion)
    original_mask = Image.open(mask_path).convert("L").resize((256, 256))
    mask_array_orig = np.array(original_mask)
    mask_array_orig = (mask_array_orig > 127).astype(np.uint8) * 255
    ...
    # Use original mask (before inversion) for correct visualization
    original_array = np.array(original_image)
    # Original mask format: white (255) = masked, black (0) = known
    # mask_array_orig is already binary (0 or 255), so check for white pixels
    # White pixels (255) = masked regions, should be shown as black in visualization
    mask_binary = (mask_array_orig == 255).astype(np.uint8)  # 1 where mask is white (masked)
    masked_array = original_array.copy()
    # Apply mask: where mask is white (masked), set image pixels to black
    masked_array[mask_binary == 1] = 0
...
def process_dataset(
    ...
    for mask_level in mask_levels:
        ...
        # Process images
        # For 60% mask, if we need image 10 specifically, use index 2 (0, 1, 10...)
        if mask_level == 60 and num_images == 1 and len(image_files) > 2:
            # Check if we already have 0 and 1, then process image 10
            selected_images = [image_files[2]]  # This will be "10.jpg" after sorting
        else:
