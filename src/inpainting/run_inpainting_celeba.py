...
Works with coordinate-based masks (.npz files).
Computes and saves mean metrics and timing information (in milliseconds).
"""
...
from core.utils import (
    apply_mask_to_image, knn_interpolation, 
    navier_stokes_inpainting, compute_metrics, 
    load_image, format_time
)

# Default configuration (can be overridden via command-line arguments)
DEFAULT_BASE_PATH = "/Users/aliag/Desktop/PhD/Classes/Introduction_to_Machine_Learning/EECE5644PROJECT"
DEFAULT_DATASET_PATH = os.path.join(DEFAULT_BASE_PATH, "dataset")
DEFAULT_INPUT_ORIGINAL_ROOT = os.path.join(DEFAULT_DATASET_PATH, "celeba_hq_256")
DEFAULT_MASK_COORDS_ROOT = os.path.join(DEFAULT_DATASET_PATH, "mask_coords")
DEFAULT_OUTPUT_ROOT = os.path.join(DEFAULT_DATASET_PATH, "inpainting_results")
...
# Default values
DEFAULT_N_IMAGES = 100
DEFAULT_N_IMAGES_TO_VISUALIZE = 100
...
def load_mask_coords(mask_coord_path):
    """Load mask coordinates from .npz file."""
    data = np.load(mask_coord_path, allow_pickle=True)
    coords = data['coords']
    meta = eval(data['meta'].item())
    return coords, meta


def create_mask_from_coords(coords, image_size=256):
    """Create binary mask from coordinate array."""
...
def get_mask_coord_filename(image_filename):
    """Get the corresponding mask coordinate filename for an image."""
    stem = Path(image_filename).stem
    return f"{stem}.npz"


def process_image(original_path, mask_coord_path, mask_level):
    """
    ...
    # Load mask coordinates and create binary mask
    coords, meta = load_mask_coords(mask_coord_path)
    mask = create_mask_from_coords(coords, image_size=TARGET_SIZE[0])
...
def create_comprehensive_comparison(all_data, image_filename, output_root):
    ...
    fig.suptitle(f'Baseline Inpainting Methods Comparison - CelebA Dataset', 
    ...
    for row_idx, mask_level in enumerate(MASK_LEVELS):
        ...
        if row_idx == 0:
            axes[row_idx, 7].set_title('Navier-Stokes\nError', fontsize=11, fontweight='bold')
...
def save_summary_statistics(all_metrics_by_level, all_timings_by_level, output_root):
    """Save summary statistics for all mask levels (timing in milliseconds)."""
    ...
    with open(summary_path, 'w') as f:
        f.write("INPAINTING METHODS - SUMMARY STATISTICS (CelebA Dataset)\n")
        f.write("=" * 80 + "\n\n")
        n_images = len(all_metrics_by_level[MASK_LEVELS[0]]) if MASK_LEVELS and len(all_metrics_by_level[MASK_LEVELS[0]]) > 0 else 0
        f.write(f"Total images processed: {n_images}\n")
    ...
    print("\n" + "=" * 80)
    print("SUMMARY STATISTICS - CELEBA DATASET")
    print("=" * 80)
...
def plot_summary_statistics(all_metrics_by_level, all_timings_by_level, output_root):
    """Create plots showing mean metrics and timing (in ms) across mask levels."""
    method_names = ['knn_uniform', 'knn_distance', 'navier_stokes']
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run inpainting methods on CelebA dataset")
    ...
    parser.add_argument("--mask_coords_dir", type=str, default=DEFAULT_MASK_COORDS_ROOT,
                       help=f"Root directory containing mask coordinate subdirectories (default: {DEFAULT_MASK_COORDS_ROOT})")
    ...
    mask_coords_root = args.mask_coords_dir
    output_root = args.output_dir
    n_images = args.n_images
    n_images_to_visualize = args.n_visualize
    
    print("=" * 80)
    print("Inpainting Evaluation - CelebA Dataset")
    print("=" * 80)
    print(f"Input directory: {input_original_root}")
    print(f"Mask coordinates root: {mask_coords_root}")
    ...
    for img_idx, image_file in enumerate(images_to_process):
        ...
        for mask_level in MASK_LEVELS:
            ...
            # Path to coordinate-based mask directory
            mask_coord_dir = os.path.join(mask_coords_root, str(mask_level))
            
            if not os.path.exists(mask_coord_dir):
                print(f"    ERROR: Mask directory not found: {mask_coord_dir}")
                continue
            
            original_path = os.path.join(input_original_root, image_file)
            
            # Get corresponding mask coordinate file
            mask_coord_filename = get_mask_coord_filename(image_file)
            mask_coord_path = os.path.join(mask_coord_dir, mask_coord_filename)
            
            if not os.path.exists(mask_coord_path):
                print(f"    ERROR: Mask coordinate file not found: {mask_coord_path}")
                continue
            
            try:
                original_img, masked_img, results, mask, metrics, timings = \
                    process_image(original_path, mask_coord_path, mask_level)
                
                # Print metrics (timing in milliseconds)
                ...
                # Save individual results (optional - uncomment if needed)
    ...
    print(f"  - Comprehensive comparisons: {output_root}/comprehensive_comparisons/ ({n_images_to_visualize} images)")
    print("=" * 80)
